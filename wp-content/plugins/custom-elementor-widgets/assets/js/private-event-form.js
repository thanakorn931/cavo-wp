/**
 * Private events, request a proposal.
 *
...
( function () {
	...
	function setUp( root ) {
		...
		var form = root.querySelector( '.custom-private-form__form' );
		var send = root.querySelector( '.custom-private-form__send' );
	...
	function start() {
		var roots = document.querySelectorAll( '.custom-private-form' );
	...
	window.addEventListener( 'elementor/frontend/init', function () {
		if ( window.elementorFrontend && window.elementorFrontend.hooks ) {
			window.elementorFrontend.hooks.addAction(
				'frontend/element_ready/private-event-form.default',
				function ( $scope ) {
					setUp( $scope[ 0 ].querySelector( '.custom-private-form' ) );
				}
			);
		}
	} );
